...
function createNav(textBtn) {
  ...
  appendToParent(logoWrapper, [logo]);
  ...
  let LoginCartDiv = createElement("div", ["LoginCartDiv"]);
  let LoginButton = createElement("button", ["LoginBtn"], textBtn || "Login");
  let divCart = createElement("div");

  if (textBtn !== "Logout") {
    const CartIcon = createIcon("fa-shopping-cart");
    appendToParent(divCart, [CartIcon]);
    divCart.classList.add("cart-icon");
  ...
  LoginButton.addEventListener("click", () => {
    if (textBtn === "Logout") {
      ...
      window.location.href = "/src/pages/seller/index.html";
    }
  });

  appendToParent(LoginCartDiv, [divCart, LoginButton]);
  appendToParent(nav, [logoWrapper, mainMenu, LoginCartDiv]);
